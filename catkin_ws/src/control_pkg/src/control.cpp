...
RouteStatus::RouteStatus(const ros::NodeHandle &_nh) : nh(_nh)
{
    route_sub = nh.subscribe(EWOK_TOPIC, 1, &RouteStatus::route_CallBack, this);
    newest_route_msg = "[ INFO] Nothing to show.\n";
}
...
void RouteStatus::route_CallBack(const std_msgs::String::ConstPtr &_msg)
{
    std::string msg = std::string(_msg->data.c_str());

    newest_route_msg = msg;
    std::cout << msg << std::endl;
}

std::string RouteStatus::routeMessageFoward()
{
    return newest_route_msg;
}